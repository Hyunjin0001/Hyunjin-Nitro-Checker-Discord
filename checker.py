...
for i in range(len(nitros)):
    ...
    url = requests.get(f"https://discordapp.com/api/v6/entitlements/gift-codes/{nitro}", proxies=ProxyParam, timeout=6)
    if url.status_code == 200:
        with open('nitrovalidecodes.txt', 'w') as nitrovalidfile:
